module ram_to_display(//读ram数据到显示屏模块
   ...
   (
    ...
    assign field_h_addr = ( (h_axis >= 1 ) && ( h_axis <= DISP_IMAGE_W ) ) ? 1 : 0 ;
    wire field_v_addr ;//要显示区域的行范围
    assign field_v_addr = ( (v_axis >= 0 ) && ( v_axis < DISP_IMAGE_H ) ) ? 1 : 0 ;
    //显示区域的范围
    wire my_field_addr ;
    ...
    assign field_h_data = ( (h_axis >= 1 ) && ( h_axis <= DISP_IMAGE_W ) ) ? 1 : 0 ;
    wire field_v_data ;//要显示区域的行范围
    assign field_v_data = ( (v_axis >= 0 ) && ( v_axis < DISP_IMAGE_H ) ) ? 1 : 0 ;
    //显示区域的范围
    wire my_field_data ;
    ...
    assign field_h_display = ( (h_axis >= 1 ) && ( h_axis <= DISP_IMAGE_W ) ) ? 1 : 0 ;
    wire field_v_display ;//要显示区域的行范围
    assign field_v_display = ( (v_axis >= 0 ) && ( v_axis < DISP_IMAGE_H ) ) ? 1 : 0 ;
    assign read_ram_request = ( field_h_display && field_v_display && ( data_request ) ) ? 1 : 0 ;
   ...
   //显示屏控制器的数据输入信号
    assign display_data = ( my_field_data ) ? ( data ) : 16'hffff;
endmodule