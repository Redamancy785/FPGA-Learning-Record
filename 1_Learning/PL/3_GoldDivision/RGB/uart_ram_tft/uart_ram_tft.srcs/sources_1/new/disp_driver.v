...
module disp_driver(
  ...
  parameter hdat_begin = `H_Sync_Time + `H_Back_Porch + `H_Left_Border ;
  parameter hdat_end = `H_Total_Time - `H_Right_Border - `H_Front_Porch ;
  
  parameter vdat_begin  = `V_Sync_Time + `V_Back_Porch + `V_Top_Border ;
  parameter vdat_end = `V_Total_Time - `V_Bottom_Border - `V_Front_Porch ;
...
always@(posedge ClkDisp)
    DataReq <= (((hcount_r + 2)>= hdat_begin)&&((hcount_r + 2  ) < hdat_end))&&
